...
bool AppInit(int argc, char* argv[])
{
    ...
    try
    {
        ...
        // Parameters
        //
        // If Qt is used, parameters/dicecoin.conf are parsed in qt/bitcoin.cpp's main()
        ...
        {
            // First part of help message is specific to dicecoind / RPC client
            std::string strUsage = _("DiceCOIN version") + " " + FormatFullVersion() + "\n\n" +
                _("Usage:") + "\n" +
                  "  dicecoind [options]                     " + "\n" +
                  "  dicecoind [options] <command> [params]  " + _("Send command to -server or dicecoind") + "\n" +
                  "  dicecoind [options] help                " + _("List commands") + "\n" +
                  "  dicecoind [options] help <command>      " + _("Get help for a command") + "\n";
        ...
        for (int i = 1; i < argc; i++)
            if (!IsSwitchChar(argv[i][0]) && !boost::algorithm::istarts_with(argv[i], "dicecoin:"))
                fCommandLine = true;
...
int main(int argc, char* argv[])
{
    ...
    // Connect dicecoind signal handlers
    noui_connect();
...
std::string HelpMessage()
{
    string strUsage = _("Options:") + "\n" +
        "  -?                     " + _("This help message") + "\n" +
        "  -conf=<file>           " + _("Specify configuration file (default: dicecoin.conf)") + "\n" +
        "  -pid=<file>            " + _("Specify pid file (default: dicecoind.pid)") + "\n" +
        ...
        "\n" + _("SSL options: (see the DiceCOIN Wiki for SSL setup instructions)") + "\n" +
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. DiceCOIN is probably already running."), strDataDir.c_str()));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    printf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    printf("DiceCOIN version %s (%s)\n", FormatFullVersion().c_str(), CLIENT_DATE.c_str());
    ...
    if (fDaemon)
        fprintf(stdout, "DiceCOIN server starting\n");
    ...
    // requested to kill dicecoin-qt during the last operation. If so, exit.
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of DiceCOIN") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart DiceCOIN to complete") << "\n";
